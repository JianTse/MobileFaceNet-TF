...
# 'E:/work/data/faceRecognize/insightface_faces_umd_ms1m_vgg_112/faces_ms1m_112x112'
def parse_args():
    ...
    parser.add_argument('--tfrecords_file_path', default='/home/ssd/data/faceRecognize/faces_ms1m_112x112', type=str,
...
def load_data(db_name, image_size, args):
    ...
    for i in range(len(issame_list)*2):
        _bin = bins[i]
        #img = mx.image.imdecode(_bin).asnumpy()
        img = misc.imread(io.BytesIO(_bin))
...
def test_tfrecords():
    ...
    tfrecords_f = os.path.join(args.tfrecords_file_path, 'faces_ms1m.tfrecords')
...
def next_batch(batch_size, pattern):
    ...
    #                                        "label": tf.FixedLenFeature([], tf.int64),
    #                                    })
    features = {'image_raw': tf.FixedLenFeature(shape=(), dtype=tf.string),
...
if __name__ == '__main__':
    ...
    pattern = os.path.join(args.tfrecords_file_path, 'faces_ms1m.tfrecords')
